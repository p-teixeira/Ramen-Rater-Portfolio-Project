...
Here I show how R can be used for data manipulation, analysis, and graphing. More specifically, I demonstrate how to load the data, some examples of cleaning and manipulating (filtering, renaming columns, pivoting, merging, etc.), and look at total review count over time, ramen consumption per capita, and appearance count in the Ramen Rater's Top 10 rankings by brand.

First thing's first, loading packages the necessary packages, and then the data. Each Excel sheet is loaded as their own data frame.
...
I remember when I was compiling the data into Excel that some of the ramen styles included in the database weren't really ramen noodles (some weren't even food). Let me show you what I mean by grouping the data by the Style column and outputting the count.
...
The styles with single digit review counts (bar, bottle, can, restaurant, and toy) aren't noodles so it doesn't make sense to include these in the analysis. So we will remove them in a bit. Out of curiosity though, what *were* these items? Let's see.

```{r}
notramenstyle <- c('Bar', 'Bottle','Can','Restaurant', 'Toy')
notramen <- ramenrev[ramenrev$Style %in% notramen, ]
...
Before we drop those though, I also want to filter out some countries with low ramen entries to remove some of the outliers. First let's see what the review distribution is per country ID.
...
Let's focus our analysis to countries with at least 50 noodle entries in the database, and remove the non-ramen entries mentioned earlier.

```{r}
filterrev <- ramenrev %>% 
  filter(Style %notin% notramenstyle) %>% 
  group_by(Country_ID) %>% 
  filter(n()>=50) %>% 
  ungroup()
...
Great. Before moving on, let's pull up the URLs for the non-ramen entries. The links are in the ramenurl data frame, so we will merge that with the notramen data frame binding on the Review_ID column.
...
As a quick first exercise, let's look at the review count over time, using the ramenrev data frame. Note that we will only use the year part of the Review_Date column. We are also removing 2022 entries as the year is still ongoing.
...
Next, something a little more involved. We will calculate the noodle consumption per capita. For this we need the country population data in the ramenctr data frame and the noodle consumption by country data in the ramencon data frame.

So let's combine the two data frames. We will do a left join (all.x=TRUE) here because the ramencon data frame includes data on countries that don't appear in the ramen review database (ramenctr on the other hand, only includes countries that have at least one noodle review, with exception of the 'World' population entry which we will drop).
...
Let's make sure the data is as expected before we proceed by check the data types for the columns

```{r}
str(ctrcon)
```

For whatever reason, the 2016 and 2017 noodle consumption columns are character type instead of numeric. Let's change that.
...
Since we don't have per ramen consumption data across all years for all countries, some of the per capita values appear as NaN. Having those in there will interfere with the graphing, so let's get rid of them (we didn't do this earlier in the ctrcon data frame because that would mean dropping the entire row of the country with missing data).

```{r}
concap <- na.omit(concap)
```

Since this is only per capita data, we don't need such wordy entries in the Year column. Let's change it so that only the year is listed
...
Finally, let's do some graphing. Let's graph the average consumption per capita per year, grouping by region. We will use ggplotly to add labels.
